...
public class QuadTree<T>
{
    ...
    static Branch CreateBranch(QuadTree<T> tree, Branch parent, int branchDepth, ref Quad quad)
    {
        ...
        float midX = quad.Min.X + (quad.Max.X - quad.Min.X) * 0.5f;
        float midY = quad.Min.Y + (quad.Max.Y - quad.Min.Y) * 0.5f;
        branch.Quads[0].Set(quad.Min.X, quad.Min.Y, midX, midY);
        branch.Quads[1].Set(midX, quad.Min.Y, quad.Max.X, midY);
        branch.Quads[2].Set(midX, midY, quad.Max.X, quad.Max.Y);
        branch.Quads[3].Set(quad.Min.X, midY, midX, quad.Max.Y);
        return branch;
    }
    ...
    public Stack<int> freeIndexes = new Stack<int>();
    public int GetFreeIndex()
    {
        int freeIndex;
        if (freeIndexes.Count > 0)
        {
            freeIndex = freeIndexes.Pop();
        }
        else
        {
            freeIndex = nextIndex;
    ...
    private void AddFreeIndex(int index)
    {
